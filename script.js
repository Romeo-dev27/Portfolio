document.addEventListener('DOMContentLoaded', () => {
    ...
    // ==========================================
    // 2. DASHBOARD LOGIC (Todos, Habits, Pomodoro)
    // ==========================================
    ...
    canvas.addEventListener('mousedown', (e) => {
        ...
        if (currentTool === 'pencil') {
            ...
            ctx.globalCompositeOperation = 'source-over';
            ctx.globalAlpha = 1.0;
            ctx.strokeStyle = '#ffffff'; // drawing white
        }
    });
    ...
    function handleTextTool(e) {
        if (textInput.style.display === 'block') return; // Already typing
        const pos = getMousePos(e);
        const rect = canvas.getBoundingClientRect();
    ...
    const saveCurrentNote = () => {
        ...
        if (note) {
            ...
            saveNotesData();
            // Don't re-render list on every keystroke to prevent losing focus, just update object
        }
    };
    ...
    noteEditor.addEventListener('mouseup', saveCurrentNote); // For formatting clicks
    ...
    document.getElementById('ed-image').addEventListener('click', () => {
